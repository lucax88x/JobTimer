...
namespace App {
    angular.module("app", ["core", "ngAnimate", "ui.router"])
        .config(["$stateProvider", "$locationProvider", "$urlRouterProvider", "initializerProvider", ($stateProvider: ng.ui.IStateProvider, $locationProvider: ng.ILocationProvider, $urlRouterProvider: ng.ui.IUrlRouterProvider, initializer) => {
        ...
        .run(["$rootScope", "$state", "$stateParams", "ajaxer", "$location", function($rootScope, $state, $stateParams, ajaxer: Core.Ajaxer, $location) {

            let speed = 100;

            $rootScope.$on("$stateChangeStart", (event, toState) => {
                let $view = $("#ui-view-hidden");
            ...
            $rootScope.$on("$stateChangeCancel", (event, to, toParams, from, fromParams, error) => {
            });

            $rootScope.$on("$stateNotFound", (event, to, toParams, from, fromParams, error) => {
                $state.go("404");
            });
